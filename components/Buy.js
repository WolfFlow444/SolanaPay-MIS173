...
export default function Buy({ itemID }) {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (result.confirmationStatus === "confirmed" || result.confirmationStatus === "finalized") {
            ...
            addOrder(order);
            setLoading(false);
            alert("Thank you spinning. You won a Free Trip to California!");
          }
        } catch (e) {
  ...
  return (
    <div>
       {status === STATUS.Paid ? (
        <IPFSDownload
          filename="Trip Itenary.pdf"
          hash="Qmc5uKbhYbt5M5T8Bg8aoKetNLuUPjpjnFG8oyBveha86k"
          cta="Download Trip Itenary"
        />
      ) : (
